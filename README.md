...
| 3. 후보 생성 + 스코어링 | ⬜ 미착수 | 여러 이미지 통합 안정 위치, 옆 슬롯 침범 방지 등 고도화 |
...
35개 테스트, 실제 샘플 CCTV 프레임(`no_label/`)을 사용하므로 이 저장소에
...
- 앞유리 탐지는 실제 프레임 1장 기준으로 튜닝한 고정 threshold — 같은 프레임에서
  차량 1대에 배경 노이즈(그림자 등) blob이 18개까지 잡힘, 슬롯 안에서 가장 큰
  blob을 고르는 방식으로만 방어 중. 조명/차종 다양한 데이터 쌓이면 적응형
  threshold로 교체 필요.
...
- 여러 이미지 일괄 처리, 옆 슬롯 침범 방지, GUI 전부 없음 — 이미지·슬롯
  1개씩만 처리 가능.
